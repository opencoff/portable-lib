/* vim: ts=4:sw=4:expandtab:notextmode:
 *
 * jenkins_hash.c - Bob Jenkins' hash function
 *
 ...
 * Licensed under same terms as Bob Jenkins' original file.
 *
 ...
 */
ub4
jenkins_hash(const void* pdata, size_t length, uint32_t initval)
{
   register ub1* k = (ub1 *)pdata;
 ...
 *
 * @doc This works on all machines.  hash2() is identical to hash() on 
 ...
 */
ub4
jenkins_hash_long(ub4 * k, size_t length, size_t initval)
{
   register ub4 a,b,c,len;
