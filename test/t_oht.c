/*
 * Test for open addressed hash table.
 *
...
static uint64_t
find_all(strvect* v, oht* h, int exp)
{
    size_t i;
    word* w;
    uint64_t tot     = 0;
    ...
    VECT_FOR_EACHi(v, i, w) {
        void *x = 0;
        uint64_t t0 = now();
        int r = oht_find(h, w->h + perturb, &x);
        tot += now() - t0;
        if (exp) {
            if (!r) {
                printf("** I-MISS %s\n", w->w);
...
static uint64_t
del_all(strvect* v, oht* h, int exp)
{
    word* w;
    uint64_t tot = 0;
    int r;
